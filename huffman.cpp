...
void huffman::create_pq()
{
	...
	if (!in_file.is_open())
	{
		cerr << "Error: could not open input file \"" << in_file_name << "\"" << endl;
		exit(1);
	}
	in_file.get(id);
	while (!in_file.eof())
...
void huffman::create_huffman_tree()
{
	...
	{//special case: file contains only one distinct character.
	 ...
	 //of leaving `root` uninitialized (which used to crash here).
		root = new huffman_node;
		root->freq = temp.top()->freq;
		root->left = temp.top();
		root->right = new huffman_node;			//unused placeholder leaf, frequency 0
		root->right->freq = 0;
		root->right->id = 0;
		temp.pop();
		return;
...
void huffman::coding_save()
{
	...
	{
		cerr << "Error: could not open input/output file for encoding" << endl;
		exit(1);
	}
	string in = "", s = "";
	...
	ifstream check(in_file_name, ios::ate | ios::binary);
	streampos original_size = check.tellg();
	check.close();
	streampos compressed_size = static_cast<streampos>(in.size());
	double ratio = original_size > 0
		? 100.0 * (1.0 - static_cast<double>(compressed_size) / static_cast<double>(original_size))
		: 0.0;
...
void huffman::recreate_huffman_tree()
{
	...
	if (!in_file.is_open())
	{
		cerr << "Error: could not open compressed file \"" << in_file_name << "\"" << endl;
		exit(1);
	}
